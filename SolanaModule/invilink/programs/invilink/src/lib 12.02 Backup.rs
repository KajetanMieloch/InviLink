...
pub mod invilink {
    ...
    pub fn validate_ticket(ctx: Context<ValidateTicket>, ticket_id: [u8; 64]) -> Result<()> {
    ...
    pub fn mark_ticket_used(ctx: Context<MarkTicketUsed>, ticket_id: [u8; 64]) -> Result<()> {
        let ticket = &mut ctx.accounts.ticket;
        require!(ticket.ticket_id == ticket_id, ErrorCode::InvalidTicket);
        require!(!ticket.used, ErrorCode::TicketAlreadyUsed);
        ticket.used = true;
...
pub struct MintTicket<'info> {
    #[account(init, payer = owner, space = 250)]
