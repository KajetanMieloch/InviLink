...
def async_generate_and_publish(event_id, section, row, seat, name, raw_date):
    try:
        ...
        # Log to file or DB if needed
        print(f"Ticket uploaded: ipfs://{cid_ticket}")
        print(f"Metadata uploaded: ipfs://{cid_metadata}")
...
def generate_metadata(request):
    ...
    if request.method == "POST":
        try:
            ...
            threading.Thread(
                target=async_generate_and_publish,
                args=(event_id, section, row, seat, name, raw_date),
                daemon=True
            ).start()

            # Return immediately with a placeholder
            ticket_id = f"{event_id}_{row}_{seat}"
            return JsonResponse({"status": "started", "ticketId": ticket_id}, status=202)
